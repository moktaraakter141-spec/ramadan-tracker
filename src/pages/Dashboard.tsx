...
const Dashboard = () => {
  ...
  const toggleMutation = useMutation({
    ...
    onMutate: async ({ habitId, day, completed }) => {
      ...
      const prev =
        queryClient.getQueryData<HabitEntry[]>(["habit_entries"]) || [];
      if (completed) {
    ...
    onError: (_err, _vars, context) => {
      if (context?.prev)
        queryClient.setQueryData(["habit_entries"], context.prev);
    },
    onSettled: () =>
  ...
  const addHabitMutation = useMutation({
    mutationFn: async (name: string) => {
      ...
      if (customCount >= 10)
        throw new Error("সর্বোচ্চ ১০টি কাস্টম অভ্যাস যোগ করা যায়");
  ...
  const fullyCompletedDays = Array.from(
    { length: TOTAL_DAYS },
    (_, i) => i + 1,
              ...
              {Array.from({ length: TOTAL_DAYS }, (_, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[9px] mt-1 font-medium ${pct === 100 ? "text-primary" : "text-muted-foreground"}`}
                    >
                      {toBanglaNum(filled)}
                    </span>
                  </button>
            ...
            {selectedDay !== null && (
              <div className="mt-3 pt-3 border-t border-border">
                <p className="text-xs font-medium mb-2">
                  দিন {toBanglaNum(selectedDay)} —{" "}
                  {toBanglaNum(
                    habits.filter((h) => isChecked(h.id, selectedDay)).length,
                  )}
                  /{toBanglaNum(habits.length)} সম্পন্ন
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                  {habits.map((h) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-2 h-2 rounded-full shrink-0 ${done ? "bg-primary" : "bg-border"}`}
                        />
                        <span
                          className={
                ...
                <thead>
                  <tr>
                    ...
                    {Array.from({ length: TOTAL_DAYS }, (_, i) => (
                      <th
                        key={i}
                        className="px-1 py-2 text-center text-xs font-medium text-muted-foreground border-b border-border min-w-[40px]"
                      >
                        {toBanglaNum(i + 1)}
                      </th>
                    ))}
          ...
          >
            <div className="space-y-3 py-4">
              ...
              <Input
                ...
                placeholder="অভ্যাসের নাম লিখুন"
              />
              <p className="text-xs text-muted-foreground">
                সর্বোচ্চ ৪০ অক্ষর •{" "}
                {toBanglaNum(habits.filter((h) => h.is_custom).length)}/১০
                কাস্টম অভ্যাস
              </p>
            </div>
            <DialogFooter>
